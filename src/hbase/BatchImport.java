...
public class BatchImport {
	...
	static class BatchImportReducer extends TableReducer<LongWritable, Text, NullWritable> {
		protected void reduce(LongWritable key,java.lang.Iterable<Text> values, Context context)
			...
			for (Text text : values) {
				...
				final Put put = new Put(splited[0].getBytes());
				//put.add(Bytes.toBytes("cf"), Bytes.toBytes("date"),Bytes.toBytes(splited[1]));
				put.add("cf".getBytes(),"date".getBytes(),splited[1].getBytes());
				//put.add("cf".getBytes(), "msisdn".getBytes(), splited[2].getBytes());
				// 省略其他字段，调用put.add(....)即可
	...
	public static void main(String[] args) throws Exception {
		...
		final Job job = new Job(configuration, "BatchImport");
